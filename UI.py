...
class Application(Frame):
    ...
    def createWidget(self):
        ...
        # 四个个按钮
        Button(self.f1, text="Select Picture", font=("SimHei", 15), command=self.selectPic).place(relx=0.02, rely=0.08,
                                                                                                 relwidth=0.12,
                                                                                                 relheight=0.1)
        Button(self.f1, text="Take Photos", font=("SimHei", 15), command=self.capture).place(relx=0.02, rely=0.25,
                                                                                              relwidth=0.12,
                                                                                              relheight=0.05)

        self.btnQuit = Button(self.f1, text="Exit",font=("SimHei", 15), command=root.destroy).place(relx=0.02,rely=0.85,
                                                                                    relwidth=0.12,
                                                                                    relheight=0.1)

        self.sep = Button(self.f1,text="Manual \nInterception \nRecognition",font=("SimHei",15),command=self.seperate).place(relx=0.02,rely=0.43,
                                                                                    relwidth=0.12,
                                                                                    relheight=0.1)

        self.btn_reg = Button(self.f1, text="Automatic  \nRecognition", font=("SimHei", 15))
        self.btn_reg["command"] = self.reg_pho
        self.btn_reg.place(relx=0.02, rely=0.6, relwidth=0.12, relheight=0.1)

        # 选择摄像头
        self.v1 = StringVar()
        self.v1.set("Local Camera")
        self.om = OptionMenu(self.f1, self.v1, "USB Camera", "Local Camera")
        self.om["width"] = 10
        self.om.place(relx=0.02, rely=0.30, relwidth=0.12, relheight=0.05)

        # 显示输入图片
        self.label01 = Label(self.f1, text="Display input picture", font=("STHeiti", 13), bg="GhostWhite")
        self.label01.place(relx=0.17, rely=0.08, relwidth=0.42, relheight=0.87)

        # 显示结果图片
        self.label02 = Label(self.f1, text="Display recognition picture", font=("STHeiti", 13), bg="GhostWhite")
        self.label02.place(relx=0.60, rely=0.08, relwidth=0.35, relheight=0.45)

        self.label03 = Label(self.f1,text="Display the identified \nmobile phone number",font=("STHeiti", 13), bg="GhostWhite")
        self.label03.place(relx=0.60, rely=0.57, relwidth=0.35, relheight=0.17)

        self.label04 = Label(self.f1, text="Display and identify \nthe mobile phone number \nintercepted manually", font=("STHeiti", 13), bg="GhostWhite")
    ...
    def selectPic(self):
        ...
        self.label03["text"] = ""
        # 显示选中的图片名字
        self.img_jpg = askopenfilename(title="Upload file", initialdir="E://gui/images", filetypes=[("Picture file", ".jpg")])
    ...
    def capture(self):
        # print(self.v1.get())
        cap_flag = 0
        if self.v1.get() == "Local Camera":  # 0
            cap_flag = 0
            print("Call the local camera to shoot")
        elif self.v1.get() == "USB Camera":  # 1
            cap_flag = 1
            print("Call USB external camera for shooting")
    ...
    def reg_pho(self):
        ...
        args = (time3, res , loc2 , 11) #将所有参数传入元组
        jsonStr = {
            "customer_phone": res,
            "phone_length": 11,
            "time" : time3,
        ...
        mysqlUpdate(args) #将该元组传入更新数据库的方法
        res = "Phone Number：" + res
    ...
    def closeWindow(self):
        ans = askyesno(title='Warning', message='Are you sure you want to close the window？')
        if ans:
...
# 连接数据库并执行更新操作
def mysqlUpdate(args):
    conn = pymysql.connect(
        host="121.199.162.167",
        port=3306,
        user="root",
        password="V1rus520.",
    ...
    cursor = conn.cursor()
    cursor.execute('INSERT INTO recognition(time, number, location, length) VALUES (%s,%s,%s,%s)', args) #采用预编译表达式防止sql注入
    conn.commit()
    cursor.close()
    conn.close()
...
if __name__ == '__main__':
    ...
    root.iconbitmap('picture/aut_logo.ico')
    root.title("Express Bill Handwriting Phone Number Recognition System")
    root["bg"] = "white"
    app = Application(master=root)
    ####
    root1 = Toplevel()
    root1.title('Free Screen Capture')
    ...
    # ================== Layout screen capture button ====================================
    button_screenShot = Button(root1, text='Screen Shot', command=screenShot)
    button_screenShot.pack()

    button_showCropImg=Button(root1,text='Display Picture',command=show)
    button_showCropImg.pack()
    # label_Img=Label(root,text='display picture')

    button_rec=Button(root1, text='Recognize', command=recognition)
    button_rec.pack()
